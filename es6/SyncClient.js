...
export class SyncClient {
  ...
  start(sendFunction, receiveFunction, reportFunction) {
    ...
    receiveFunction('data', (data) => {
      if (data.msg == 'sync:pong') {
        let pingId = data.args[0];
        let clientPingTime = data.args[1];
        let serverPingTime = data.args[2];
        let serverPongTime = data.args[3];
        ...
        if (pingId === this.pingId) {
          ...
          this.streakData[this.streakDataNextIndex] = [travelDuration, offsetTime, clientTime, serverTime];
          ...
          // end of a streak
          if (this.pingStreakCount >= this.pingStreakIterations && this.streakData.length >= this.streakDataLength) {
            // plan the begining of the next streak
            this.pingDelay = this.pingStreakDelay.min + Math.random() * (this.pingStreakDelay.max - this.pingStreakDelay.min);
            ...
            this.longTermData[this.longTermDataNextIndex] = [streakTravelDuration, streakClientTime, streakServerTime,
            ...
            if ((this.status === 'training' && this.getStatusDuration() >= this.longTermDataTrainingDuration) || this.status === 'sync') {
              ...
              if (variance > 0) {
                ...
                if (this.frequencyRatio > 0.99 && this.frequencyRatio < 1.01) {
                  ...
                  this.longTermData[0] = [streakTravelDuration, streakClientTime, streakServerTime,
                    streakClientSquaredTime, streakClientServerTime
                  ];
